...
# Enable debug output only after sensitive varsVERSIONve been set, to reduce risk of leak
DEBUG=${DEBUG:-''}
...
function downloadAndroidDependencies() {
  ...
  if ! ls ".tmp/pixincreate-$MAGISK_VERSION.apk" >/dev/null 2>&1 && [[ "${POTENTIAL_ASSETS['pixincreate']+isset}" ]]; then
    curl --fail -sLo ".tmp/pixincreate-$MAGISK_VERSION.apk" "https://github.com/pixincreate/Magisk/releases/download/$MAGISK_VERSION-3/Magisk-$MAGISK_VERSION-3.apk"
  fi
